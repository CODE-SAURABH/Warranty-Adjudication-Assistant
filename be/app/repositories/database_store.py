...
def delete_prior_repair_history_records(previous_claim_id: str) -> int:
    with session_scope() as session:
        result = session.execute(
            delete(PriorRepairHistory).where(PriorRepairHistory.previous_claim_id == previous_claim_id)
        )
        return int(result.rowcount or 0)
